...
public class DatabaseOperationTest extends TestBase {
    ...
    public void testLocalDatabases()
        ...
        try {
            ...
            /* Create a single rep DB for testing mixed txns. */
            final DatabaseConfig repDbConfig =
                dbConfig.clone().setTransactional(true).setReplicated(true);
            createMixedTxnDb("repDb", master, txnlDbs, repDbConfig);
            ...
            checkAndRemoveMixedTxnDb("repDb", master, txnlDbs, repDbConfig);
    ...
    /**
     * Create and return rep DB.  Write one record with key 100 to it and the
     * txnl local DB on the master, using an explicit txn.
     */
    private void createMixedTxnDb(final String dbName,
        ...
        for (int i = 0; i < nEnvs; i += 1) {
            ...
            final Database repDb = env.openDatabase(null, dbName, dbConfig);

            /* Commit txn with key 100. */
            Transaction txn = env.beginTransaction(null, null);
            key.setData(new byte[] { (byte) 100 });
            data.setData(new byte[] { (byte) 100 });
            OperationStatus status = repDb.put(txn, key, data);
            assertSame(OperationStatus.SUCCESS, status);
            status = localDbs[i].put(txn, key, data);
            assertSame(OperationStatus.SUCCESS, status);
            txn.commit();

            /* Abort txn with key 200. */
            txn = env.beginTransaction(null, null);
            key.setData(new byte[] { (byte) 200 });
            data.setData(new byte[] { (byte) 200 });
            status = repDb.put(txn, key, data);
            assertSame(OperationStatus.SUCCESS, status);
            status = localDbs[i].put(txn, key, data);
            assertSame(OperationStatus.SUCCESS, status);
            txn.abort();
    ...
    /**
     * Check rep DB and txnl local DB on the master, then remove rep DB, using
     * an explicit txn.
     */
    private void checkAndRemoveMixedTxnDb(final String dbName,
                                          final ReplicatedEnvironment master,
                                          final Database[] localDbs,
                                          final DatabaseConfig dbConfig) {
        ...
        for (int i = 0; i < nEnvs; i += 1) {
            ...
            final Database repDb = env.openDatabase(txn, dbName, dbConfig);
            assertEquals(1, repDb.count());
            assertEquals(2, localDbs[i].count());
            for (final Database db : new Database[] {repDb, localDbs[i]}) {
                key.setData(new byte[] { (byte) 100 });
                OperationStatus status = db.get(txn, key, data, null);
                assertSame(OperationStatus.SUCCESS, status);
                assertEquals(100, data.getData()[0]);
            ...
            if (env.getState().isMaster()) {
                ...
                env.renameDatabase(txn, dbName, newName);
                env.truncateDatabase(txn, newName, false);
                env.removeDatabase(txn, newName);
            }
            txn.commit();
        }
    }
